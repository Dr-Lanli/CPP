...
int main (void)
{
    ...
    DiamondTrap dt("Unbreakable");

    std::cout << "\n=== Test attacks ===" << std::endl;
    dt.attack("training target");      // hérité de ScavTrap (via ClapTrap)
    dt.takeDamage(30);           // hérité de ClapTrap
    dt.beRepaired(20);           // hérité de ClapTrap

    std::cout << "\n=== Special mode ===" << std::endl;
    dt.guardGate();              // capacité spécifique à ScavTrap
    dt.highFivesGuys();          // capacité spécifique à FragTrap
